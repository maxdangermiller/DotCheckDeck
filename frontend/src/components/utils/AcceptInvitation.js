...
const AcceptInvitation = (props) => {
    ...
    const activateAccount = () => {
        ...
        axios({
            method: 'POST',
            url: WINDOW_LOCATION + '/activate-invited-user',
            data: {
    ...
    useState(() => {
        try {
            fetch(WINDOW_LOCATION + '/invited-user-auth/' + enc_key, {
                    method: 'GET',
                    headers: {
                        'Content-type': 'application/json; charset=UTF-8',
                    }
            ...
            .then((response) => {
                if (response.status === 200) {
                    ...
                    setShowAlert(true);
                    setAlertText("Invalid invitation key!");
                }
            })
            .catch((error) => {
                setShowAlert(true);
					...
					<div className="col-14 col-md-10 col-lg-8 col-xl-6">
						<div className="card bg-dark text-white" style={{borderRadius: '1rem', height: "80vh"}}>
							<div className="card-body p-5 text-center h-100">
                                ...
                                <button className="btn btn-outline-light btn-lg px-5" type="submit" onClick={e => activateAccount()}>Setup Account</button>
							</div>
						</div>
					</div>
				</div>
